...
const { getMovementSpeedByType, flyingTypesMapping } = require("./unitConfig");
...
function getClosestPathablePositionsBetweenPositions(resources, position, targetPosition, gasGeysers = []) {
  ...
  const structureAtPositionCells = BuildingPlacement.getStructureCells(position, mapFixturesToCheck);
  const structureAtTargetPositionCells = BuildingPlacement.getStructureCells(targetPosition, mapFixturesToCheck);
